...
igvCilindro::igvCilindro(float r, float a, int divU, int divV)
{
	...
	vertices = new float[num_vertices * 3 * r];
	normales = new float[num_vertices * 3 * r]; /*Mismo numero de normales que de vertices.*/
	float i = 0; int k = 0;
...
int igvCilindro::creaCircunferencia(float _altura, float r, int divU, int k) {
	...
	while (radianes < 2 * PI * r) { /*Lo dejo en < estricto ya que debe de ser por igual el calculo del radian con valor 0 que con valor 2PI*R */
		glTexCoord2f(cos(radianes) * r, sin(radianes) * r); //Coordenadas (x,z) del cilindro ya que la y no cambia
		vertices[k] = cos(radianes) * r;
		normales[k++] = vertices[k] / r;
		vertices[k] = _altura * r;
		normales[k++] = 0;
	...
	vertices[k] = _altura * r;
	normales[k++] = 0;
	vertices[k] = vertices[2];
	normales[k++] = vertices[2] / r;
