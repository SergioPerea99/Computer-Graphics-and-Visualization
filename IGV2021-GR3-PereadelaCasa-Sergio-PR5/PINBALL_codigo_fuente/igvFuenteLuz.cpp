...
void igvFuenteLuz::aplicar(void) {
	...
	if (encendida) { // si la luz está encendida
		...
		glLightfv(idLuz, GL_POSITION, posicion.cloneToFloatArray());//	establecer la posición de la luz
		...
		glLightf(idLuz, GL_SPOT_EXPONENT, exponente_foco); //DUDA: ¿este exponente_foco no debería ser un integer en vez double? o igualmente vale mientras esté en el intervalo [0,128] ?
